...
If a local file `img/pdf_logo.png`, `img/pdf_logo.jpg` or `img/pdf_logo.jpeg` exists, it is automatically preferred for the PDF. Otherwise the PDF falls back to `img/logo.png`, `img/logo.jpg` or `img/logo.jpeg`. 
...
If `server_url` is configured in `config/mailconfig.php`, the submission is also forwarded as JSON to that endpoint. This callback is optional and should only be set when it is really needed.
